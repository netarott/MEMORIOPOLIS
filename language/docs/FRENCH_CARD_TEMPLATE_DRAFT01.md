# French card template

## Front
```html
<div class="concept-id">{{ID}}</div>
<div class="layer-name">Français</div>
<div class="target-word">{{French}}</div>
{{#Pronunciation}}<div class="pronunciation">/{{Pronunciation}}/</div>{{/Pronunciation}}
<div class="tts-audio">{{tts fr_FR:French}}</div>
```
...
{{#ExampleFrench}}<div class="section example-section"><div class="label">例文・Français</div><div class="example-text">{{ExampleFrench}}</div></div><div class="tts-audio">{{tts fr_FR:ExampleFrench}}</div>{{/ExampleFrench}}
...
{{#SoundBridge}}<div class="section sound"><div class="label">音と連結の橋</div><div class="content">{{SoundBridge}}</div></div>{{/SoundBridge}}
...
.sound { background:#e5edf2; border-left:4px solid #426f8a; }
